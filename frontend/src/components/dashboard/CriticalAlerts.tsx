...
const CriticalAlerts = ({ isTeamView, realtimeAlerts, scanAlerts, connectionStatus }: CriticalAlertsProps) => {
  const navigate = useNavigate();

  const alerts = scanAlerts && scanAlerts.length > 0
    ? scanAlerts
    : !isTeamView && realtimeAlerts
                ...
                <div className="flex items-center gap-2 mt-1 flex-wrap">
                  ...
                  {alert.memberName && (
                    <>
                      ...
                      <span className="text-xs text-muted-foreground">{alert.memberName}</span>
                    </>
                  )}
                  {alert.branch && (
                    <>
                      <span className="text-xs text-muted-foreground">-</span>
                      <span className="text-xs text-muted-foreground">{alert.branch}</span>
                    </>
                  )}
                  <span className="text-xs text-muted-foreground">-</span>
